...
<<>>=
(traits <- read.table(header = TRUE, sep = ';', text = 'taxon;T1M1;T1M2;T1M3;T2M1;T2M2
...
<<>>=
(abundances <- read.table(header = TRUE, sep = ';', text = 'taxon;abundance
Gammarus roeseli;5
Gammarus roeseli;6
Gammarus tigrinus;7
Gammarus tigrinus;8
Coenagrionidae;10
Coenagrionidae;6
Potamopyrgus antipodarum;10
'))
@

classification(gsub(' sp.', '',traits$taxon), ID='tsn')

get_uid(gsub(' sp.', '',traits$taxon))

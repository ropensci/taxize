...
# As we already have Taxonomic Serial Numbers from ITIS, let's just get classifications from ITIS. Note that we could use NCBI instead.
...
\begin{figure}[!ht] 
\centering 
<<map, eval=TRUE, dev='pdf', out.width='.8\\textwidth'>>=
library(rgbif)
library(ggplot2)

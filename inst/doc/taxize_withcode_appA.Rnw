...
# Make a map
p <- rgbif::map(occurr_list)
p <- p + guides(col = guide_legend(nrow = 2, byrow = TRUE)) + theme(legend.position="bottom")
p
@
\caption{a map...} 
\end{figure}

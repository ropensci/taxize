...
A complete reproducible workflow, from a species list to a distribution map
======

If you aren't familiar with a complete workflow in R, it may be difficult to visualize the process. In R, everything is programmatic, so the whole workflow can be in one place, and be repeated whenever necessary. The following is a workflow for taxize, going from a species list to a map. 
...
splist <- c("Helanthus annuus", "Pinos contorta", "Collomia grandiflorra")
```
...
out <- occurrencelist_many(as.character(allnames$spname), coordinatestatus = TRUE, maxresults = 10)
outdf <- gbifdata(out)
gbifmap_list(out) +

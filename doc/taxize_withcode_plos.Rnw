...
<<phylomaticphylogeny, eval=TRUE, dev=c ('pdf','tiff'), dpi=300, out.width='4in', out.height='4in', cache=TRUE, include=FALSE>>=
...
We see that the two \emph{Juncus} species are aggregated to Juncaceae and their abundances are summed up. If a taxon is one lower taxonomic resolution then the queried or the taxon is not found in the database then these are not aggregated and returned as is.
...
\end{center}
\caption{
{\bf A phylogeny for three species. This phylogeny was produced using the \emph{phylomatic\_tree} function, which queries the Phylomatic database, and prunes a previously created phylogeny of plants.}
}
...
gisd\_isinvasive & Invasiveness status & Global Invasive Species Database \cite{gisd}  \\

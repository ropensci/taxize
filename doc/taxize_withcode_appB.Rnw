...
<<traitdata, tidy=FALSE, comment=NA, message=FALSE, warning=FALSE>>=
...
<<abundancedata, tidy=FALSE, comment=NA, message=FALSE, warning=FALSE>>=
...
<<cleaning, comment=NA, message=FALSE, warning=FALSE>>=
...
<<get_classification, cache=TRUE, comment=NA, message=FALSE, warning=FALSE>>=
library(taxize)
...
<<direct_matches, comment=NA, message=FALSE, warning=FALSE>>=
...
<<higher_resolution, comment=NA, message=FALSE, warning=FALSE>>=
...
<<lower_resolution, message=FALSE, warning=FALSE>>=
...
<<na_omit, message=FALSE, warning=FALSE>>=
abundances <- abundances[!abundances$taxon == lookup$taxon[is.na(lookup$traits)], ]
@


Now we can create \emph{species x sites} and \emph{traits x species} matrices, which could be plugged into methods to analyse trait responses [27].


<<matrices, message=FALSE, warning=FALSE>>=

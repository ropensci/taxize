\section*{Installation of the development version of taxize and API keys} 
...
<<openrprofile, engine='bash', eval=FALSE>>=
open .Rprofile
@
...
<<writerprofile, engine='bash', eval=FALSE>>=
# uBio API key

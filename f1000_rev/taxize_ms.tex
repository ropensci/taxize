...
\hlstd{B1} \hlkwb{<-} \hlstr{"gammarus roeseli"}
\hlstd{B2} \hlkwb{<-} \hlstr{"gammarus"}
\hlstd{B3} \hlkwb{<-} \hlstr{"gammaridae"}
...
\hlstd{B3_clas} \hlkwb{<-} \hlkwd{classification}\hlstd{(B3,} \hlkwc{db} \hlstd{=} \hlstr{'ncbi'}\hlstd{)}
\hlstd{B1[}\hlkwd{match}\hlstd{(A, B1)]}
\end{alltt}
\begin{verbatim}
# [1] "gammarus roeseli"
\end{verbatim}
...
\end{verbatim}
\begin{alltt}
\hlstd{A_clas[[}\hlnum{1}\hlstd{]]}\hlopt{$}\hlstd{Rank[}\hlkwd{tolower}\hlstd{(A_clas[[}\hlnum{1}\hlstd{]]}\hlopt{$}\hlstd{ScientificName)} \hlopt{%in%} \hlstd{B3]}
\end{alltt}
\begin{verbatim}
# [1] "family"
\end{verbatim}
...
\hlkwd{colnames}\hlstd{(df)} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{"Bellis perennis"}\hlstd{,} \hlstr{"Juncus bufonius"}\hlstd{,} \hlstr{"Juncus articulatus"}\hlstd{)}
\hlkwd{head}\hlstd{(df)}
\end{alltt}
\begin{verbatim}

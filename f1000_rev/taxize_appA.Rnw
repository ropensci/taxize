...
<<thespecieslist, eval=TRUE>>=
splist <- c("Helanthus annuus", "Pinos contorta", "Collomia grandiflorra", "Rosa california", "Mimulus bicolour", "Nicotiana glauca", "Maddia sativa")
@
...
<<commonnames, eval=TRUE, cache=TRUE>>=
...
<<classification, eval=TRUE, cache=TRUE>>=
# As we already have Taxonomic Serial Numbers from ITIS, let's just get classifications from ITIS. Note that we could use uBio instead.
...
# Ah, so Abies and Bartlettia are in different infradivisions, but share taxonomic names above that point.
@
...
<<phylogeny, eval=TRUE, dev='pdf', out.width='4in', out.height='4in', cache=TRUE, message=FALSE, fig.cap='A phylogeny.', fig.pos='!h', fig.lp='fig:phylo'>>=
...
<<plot_map, eval=TRUE, dev='pdf', out.width='.9\\textwidth', cache=TRUE, fig.cap='A map.', fig.pos='h', fig.lp='fig:map', fig.height=7, fig.width=10>>=
library(rgbif)
library(ggplot2) 
